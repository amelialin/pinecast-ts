...
function encode(rawData) {
  let dataType = typeof rawData == 'string' ? kString : kUint8Array,
    ...
    outData = [],
    getByte =
      dataType == kString ? i => rawData.codePointAt(i) : i => rawData[i];
...
function encodeFile(inpath, outpath, options, callback) {
  ...
  let decoderScript = options.addDecoder
    ? fs.readFileSync('decode.min.js')
    : '';

  outStream.on('error', () => {
    throw 'Error writing to ' + outpath;
  });
  inStream.on('error', () => {
    throw 'Error reading from ' + inpath;
  });
  ...
  rl.on('line', line => {
    ...
    while ((results = regexp.exec(line)) !== null) {
      ...
      outStream.write('"');
      if (mimetype != kDefaultMimeType)
        outStream.write(' data-b122m="' + mimetype + '"');
    ...
    if (options.addDecoder) {
      ...
      if ((results = bodyRegExp.exec(line)) != null) {
        ...
        if (results.index >= prevIndex) {
          outStream.write(
            line.substring(0, results.index) +
              '<script>' +
              decoderScript +
              '</script>',
          );
...
function decode(base122Data) {
  let strData =
      typeof base122Data == 'string'
        ? base122Data
        : utf8DataToString(base122Data),
    decoded = [],
    decodedIndex = 0,
    curByte = 0,
    bitOfByte = 0;
