...
export function reduceReducers<T>(...reducers: Array<Reducer<T>>): (Reducer<T>) {
    return (state: T, action: Action<any>) => reducers.reduce((acc, cur) => cur(acc, action), state);
};


export function actionHandler<T>(handlers: {[key: string]: Reducer<any>}): Reducer<T> {
