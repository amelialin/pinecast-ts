...
function matcher(regexSource: string): (url: string) => RequestURLParams | null {
    ...
    const regex = new RegExp(
        '^' +
        params.reduce((acc, cur) => acc.replace(`:${cur}`, '([\\w\\-]+)'), regexSource) +
        '$'
    );
    return url => {
        ...
        return params.reduce((acc: RequestURLParams | null, cur: string, i: number) => {
            if (!acc || !output[i]) {
                return null;
            }
            acc[cur] = output[i];
